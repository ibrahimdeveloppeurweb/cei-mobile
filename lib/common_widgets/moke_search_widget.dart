...
import '../services/fake_search_service.dart' as FakeSearchService;

// Import our fake search service

class MockSearchForm extends StatefulWidget {
  final String selectedType;
  final Function(bool personFound, UserModel? user) onContinue;
  ...
  @override
  State<MockSearchForm> createState() => _MockSearchFormState();
}

class _MockSearchFormState extends State<MockSearchForm> {
  ...
  void initState() {
    super.initState();
    // Pre-fill test data in debug mode
    _prefillTestData();
  }

  void _prefillTestData() {
    // In a real app, you might want to check if in debug mode first
    bool isDebugMode = true;

    if (isDebugMode) {
      _formNumberController.text = FakeSearchService.testCredentials['form_number']!;
      _voterNumberController.text = FakeSearchService.testCredentials['voter_id']!;
      _searchFamilyNameController.text = FakeSearchService.testCredentials['lastname']!;
      _searchFirstNameController.text = FakeSearchService.testCredentials['firstname']!;
      _selectedDay = FakeSearchService.testCredentials['dob_day'];
      _selectedMonth = FakeSearchService.testCredentials['dob_month'];
      _selectedYear = FakeSearchService.testCredentials['dob_year'];
    }
  }
  ...
  String get _getTitle {
    switch (widget.selectedType) {
      ...
      case 'Candidature':
        return 'Vérifiez votre enrôlement';
      default:
  ...
  String get _getSubtitle {
    switch (widget.selectedType) {
      ...
      case 'Candidature':
        return 'Vous devez être enrôlé sur la liste électorale pour poser votre candidature.';
      default:
  ...
  String get _getInfoBannerText {
    switch (widget.selectedType) {
      ...
      case 'Candidature':
        return 'Pour poser votre candidature, vous devez d\'abord vérifier votre enrôlement sur la liste électorale.';
      default:
        return 'Veuillez rechercher la personne concernée.';
    }
  }
  ...
  Widget _buildPersonFoundCard() {
    ...
    if (widget.selectedType == 'Inscription') {
      ...
      statusText = widget.selectedType == 'Candidature'
          ? 'Enrôlement vérifié'
          : 'Personne trouvée';
    }
            ...
            children: [
              Container(
                ...
                child: ClipRRect(
                  ...
                  child: foundUser!.imageUrl.isEmptyOrNull
                      ? Image.asset(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    8.height,
                    _buildDetailRow('N° d\'ordre:', foundUser!.orderNumber.validate()),
                    8.height,
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        4.width,
                        Text(
                          foundUser!.uniqueRegistrationNumber.validate(),
      ...
      children: [
        ...
        AppButton(
          ...
          onTap: () {
            ...
            } else if (widget.selectedType == 'Candidature') {
              if (_personFound) {
                widget.onContinue(true, foundUser);
              } else {
                toast('Vous devez être enrôlé pour poser votre candidature.');
              }
            }
          },
  ...
  Future<void> _performFormNumberSearch() async {
    ...
    try {
      // Use our fake search service instead of real API
      final result = await FakeSearchService.FakeSearchService.searchByFormNumber(_formNumberController.text.trim());

      setState(() {
        _searchLoading = false;
        _personFound = true;
        foundUser = _convertToUserModel(result['data']);
      });
    } catch (e) {
      setState(() {
        _searchLoading = false;
  ...
  Future<void> _performVoterIdSearch() async {
    ...
    try {
      // Use our fake search service instead of real API
      final result = await FakeSearchService.FakeSearchService.searchByVoterId(_voterNumberController.text.trim());
  ...
  Future<void> _performNameSearch() async {
    ...
    try {
      ...
      // Use our fake search service instead of real API
      final result = await FakeSearchService.FakeSearchService.searchByNameAndBirthdate(
        _searchFirstNameController.text.trim(),
        _searchFamilyNameController.text.trim(),
        birthdate,
      );
  ...
  // Helper method to convert from fake data format to UserModel
  UserModel _convertToUserModel(Map<String, dynamic> data) {
    return UserModel(
      id: 1,
      firstName: data['firstName'] ?? '',
      lastName: data['lastName'] ?? '',
      uniqueRegistrationNumber: data['uniqueRegistrationNumber'] ?? '',
      ...
      pollingStation: data['pollingStation'] != null
          ? BureauModel(
        id: 1,
        numero: data['pollingStation']['name'] ?? '',
        centre: data['pollingStation']['centre'] != null
            ? CentreModel(
          id: 1,
          name: data['pollingStation']['centre']['name'] ?? '',
        )
            : null,
      )
          : null,
  ...
  String _getPollingStationName() {
    return foundUser?.pollingStation?.centre?.name ?? "Centre de vote non défini";
  }

  String _getFormattedDateOfBirth() {
    return foundUser?.dateOfBirth != null
        ? "${foundUser!.dateOfBirth!.day}/${foundUser!.dateOfBirth!.month}/${foundUser!.dateOfBirth!.year}"
        : "Date non disponible";
  }
}
